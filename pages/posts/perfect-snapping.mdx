...
Before we can get too far into snapping—and in order to make use of our offset, once we've found it—we'll first need to cover how dragging works. It's important that we do our dragging the right way:

To move a shape, we add a delta to its point.

<img src="/images/snapping/dragging_1.png" />

If our shapes looked like this:
...
Then with a helper function named `add`...

```ts
const add = (a: number[], b: number[]) => [a[0] + b[0], a[1] + b[1]]
```

We could move a shape like this:

```ts
function moveShape(shape: Shape, delta: number[]) {
  return {
    ...shape,
...
Where can we find this delta? Here's the trick: rather than

We need to compute the position of the pointer based on the delta (or difference) between the user's _current_ pointer and their _initial_ point; or where the pointer is _now_ and where the pointer was when the user _began_ the drag.

We can then find the selection's new position by adding this delta to the _shape's_ initial point:

<img src="/images/snapping/snapping_9.png" />

Let's get some more helpers for the math involved:
...
And then we can define a `dragShape` function like this:
...
In order to make use of our offset, we'll need to add it to the position of our dragging selection. In order for this to work correctly, it's important that we do our dragging the right way:

<img src="/images/snapping/snapping_8.png" />
...
To use our offset, we would add our offset to the `point`.
...
class App {
  ...
  startDragging = (shapes: Shape[], point: number[]) => {
    this.snapshot = {
      bounds: getSelectionBounds(this),
      targets: this.getOffsetFromSnaps(this.state)
    } =
  }
}
```
...
```ts
function dragShapeWithSnaps(
  state: State,
  initialBounds: AABB,
  point: number[],
  origin: number[]
) {
  const nextShape = dragShape(initialShape, point, origin)
  const bounds = getAABB(state)
  const others = getSnappableAABBs(state)
  const snaps = getSnaps(bounds, others)

  const offset = findSelectionSnapOffset()
