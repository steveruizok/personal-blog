---
title: "Reordering Part 2: Fractional Indexing"
date: "02/11/2022"
hero: "/images/reordering-2/send_to_back_2.png"
status: "published"
description: "Implementing reordering commands (Send to Back, Send Backward, Bring Forward, and Bring to Front) in a hash table of items."
---

In a previous blog post, we looked at implementations of common reordering-2 commands in a zoom UI or canvas app. Those commands are:
...
While the last post showed implementations in an app where items are stored in an array, this post will focus on the more complex implementation for apps where items are stored in hash tables.

## Mise en place

Let's say we have an application where we're storing items in a hash table. Because tables (unlike arrays) have no "position" or index for items in the table, we'll need to keep track of that information ourselves.
...
In this structure, each item's `index` is stored on the object itself. In the example above, the item `{ id: "a", index: 1}` has the index of `1`, the item `{ id: "b", index: 2 }` has the index of `2`, etc. Note that the lowest index is `1`, and not `0`. This is a deliberate choice and we'll come back to it in a moment.

Now that we have our data worked out, let's look at how we would implement our four reordering-2 commands.
...
The problem with this approach is that making a change to one item's order might mean making a change to _all_ items in the stack. This strategy could lead to excessive writes to a database, or larger packets being sent to ensure consistency between users in a multiplayer session, and even unnecessary renders depending on how changes are observed.

Luckily, there is a better way to represent reordering that ensures that only the _moving_ items will require new `index` values.

### Fractional Indexing

The strategy we'll be using is called "fractional indexing", introduced to me by [Figma](https://www.figma.com/blog/realtime-editing-of-ordered-sequences/#fractional-indexing). In this strategy, an `index` only needs to ensure that, when our items are sorted by their `index`, the items end up in the right order. The `index` values themselves do not have to be integers or evenly distributed—all that matters is that they let us produce the correct sort.
...
In our case, we'll be representing our fractional indices by numbers. We'll use the following function to get the indices between two other `index` values:
...
If we're putting items at the "bottom" of the list, our `above` value will be lowest `index` among our items. In that case, we'll create new fractions based on that `index`.
...
We'll see this in use when we implement `sendToBack` and `sendBackward`
...
If we're putting items at the "top" of the list, then our `below` value will be the highest `index` among our items. Here we can simply iterate by incrementing the that `index`.
...
We might see this with any of our methods.
...
One last note: in addition to our `getIndicesBetween` method, we'll also be using the following `sortByIndex` method:
...
Fractional indexing makes reordering operations much cheaper in terms of writes to the document or database, as only the mutations necessary are those for the items that need to move.
...
console.log(below, above, below < above) // 1, 1.0000000000000002, true
```
...
console.log(below, above, below < above) // 1, 1, false
```
...
console.log(Object.values(items).map((item) => item.index))
// 1, 1.0000000000000002, 1, 4, 5, 6, 7
```

In the example above, we're moving the third item from the start backward 53 times. After the 52th time, that item's index loses its precision. (On the next iteration, the item in position 1 would lose its precision as well.)

The solution to this is to use a more accurate index. Remember that the `index` value only matters insofar as it will produce the correct order when sorted. At the moment our sort compares the two numbers, however we can also sort _alphabetically_.
...
const itemExample: Item = { id: "a", order: "a0" }
...
This method for representing indices is described in detail in [an article](https://observablehq.com/@dgreensp/implementing-fractional-indexing) by David Greenspan. It works nearly indefinitely.

While I won't reproduce the code here, the good news is that the method is entirely compatible with the code in this article: all we need to do is replace our `index` as a string instead of a number, replace our `sortByIndex` to sort alphabetically instead, and make our `getIndicesBetween` method generate indices via David's method.

I've created a [separate CodeSandbox](https://codesandbox.io/s/table-items-fractional-index-examples-wj0mg) with this implementation. The methods behave in the same was as described in this article—the only difference is how indices are represented and generated.
