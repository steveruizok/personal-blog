---
title: "Copy to Multiplayer Room"
date: "05/19/2022"
hero: "/images/copy-to-liveblocks/ricky-kharawala-adK3Vu70DEQ-unsplash.jpg"
status: "published"
description: "How I used Liveblock's Storage APIs to create a new feature."
---

I recently implemented a feature in [tldraw](https://tldraw.com "tldraw") that lets a user copy the contents of their current page into a new multiplayer room. There are still some issues around images and videos (as these need to be converted and uploaded separately) but it should be a pretty handy feature if you want to share a project or collaborate on something. Just send the link!
...
export default async function CreateMultiplayerRoom(
  ...
  try {
    // 1. Create the Liveblocks storage JSON
    // 2. Get an authentication token from Liveblocks
...
This would give me an object with a `token`, the web token I'd need to communicate with Liveblocks' storage API.
...
storageJson.data.shapes.data = document.pages[pageId].shapes ?? {}
storageJson.data.bindings.data = document.pages[pageId].bindings ?? {}
...
If all went well, my API endpoint would reply to the tldraw client application with the new room's URL.
