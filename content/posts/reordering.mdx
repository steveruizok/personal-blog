...
There are several ways of implementing these four commands, and these can vary greatly depending on how an application structures its items.

In this article, we'll cover implementations in an app that uses an array to structure its items. In the a future post, we'll cover a more complex method for an application where items are stored in a hash table.
...
...and we're storing them in an array:
...
Here an item's "order" is represented by the item's index in the array. In the example above, the item `{ id: "a" }` has the index of `0`, the item `{ id: "b" }` has the index of `1`, etc.

With this as our data, let's look at how we would implement our four reordering commands.
