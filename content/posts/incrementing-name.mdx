...
We call the `getName` function with the input name(`name`) and an array of exising names (`others`) that may or may not contain matches.

In a while loop, we check if the `others` includes the current `result`.

If the `others` array includes the `name`, then we mutate the `result`: either by adding a `" (1)"` if the name isn't already incremented, or else, if the name is already incremented, by incrementing the number between the parentheses.
